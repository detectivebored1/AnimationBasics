...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    pos[numParticles] = new Vec2(30+random(60),-190-random(10));
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    vel[i].add(acc.times(dt));  // Apply gravity
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      Vec2 obstNormal = obstacleVel.normalized();                        //Obstacle velocity normal
      pos[i] = spherePos.plus(normal.times(sphereRadius+r).times(1.01));
      Vec2 velNormal = normal.times(dot(vel[i],normal));
      if(leftPressed || rightPressed || upPressed || downPressed ){ vel[i].subtract(obstNormal.times(1+COR)); }  //Added momentum to particles if obstacle is moving
