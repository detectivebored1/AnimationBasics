...
void setup(){
  ...
  agentPos[3] = new Vec2(220,420);
  agentPos[4] = new Vec2(420,610);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(100, 300);
  goalPos[4] = new Vec2(210,110);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  float ttc = rayCircleIntersectTime(pos1, radius1+radius2, pos2, vel1.minus(vel2));
...
Vec2 computeAgentForces(int id){
  ...
  goal_force = goal_force.times(k_goal);
  
  Vec2 a_future = new Vec2(0, 0); 
  Vec2 b_future = new Vec2(0, 0);
  Vec2 relVec = new Vec2(0, 0);
  float ttc = 0;
  for(int j = 0; j < numAgents; j++) {
      if(j != id) 
      {
        ...
        if(ttc >= 0) {
          a_future = agentPos[id].plus(agentVel[id].times(ttc));
          b_future = agentPos[j].plus(agentVel[j].times(ttc));
          relVec = (a_future.minus(b_future)).normalized();
        }
        else ttc = -1;
      }
  }
  ...
  if(ttc >= 0)  acc = acc.plus(relVec.times(k_avoid * (1/ttc)));
  return acc;
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, actualRad*2);
  }
}
