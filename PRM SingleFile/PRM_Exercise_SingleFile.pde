...
int closestNode(Vec2 point){
  ...
  for(int i = 1; i < numNodes; i++){
    if(point.distanceTo(nodePos[i]) < closestNodedist){
      closestNodedist = point.distanceTo(nodePos[i]);
      closeNum = i; 
    }
  }
  return int(closeNum);
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  //TODO: Return true if the point is actually inside the box4
  Vec2 dist = new Vec2(pointPos.x - boxTopLeft.x, pointPos.y - boxTopLeft.y);
  if(dist.x < boxW && dist.y < boxH && 0 < dist.x && 0 < dist.y){
    return true;
  }
  return false;
}
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideTheSquare = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (insideAnyCircle || insideTheSquare){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
      insideTheSquare = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }
    nodePos[i] = randPos;
  }
}
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if(distBetween < 200){
        //rayCircleIntesect(Vec2 center, float r, Vec2 l_start, Vec2 l_dir, float max_t)
        hitInfo circleListCheck = rayCircleListIntesect(circlePos, circleRad, nodePos[i], dir, distBetween);
        //rayBoxIntersect(Vec2 boxTopLeft, float boxW, float boxH, Vec2 ray_start, Vec2 ray_dir, float max_t)
